...
## Post-script — execution outcome (2026-07-27)

- **#1 (capacity-check `mark_delivered`) — SHIPPED, v0.5.4.** `OverDelivered` + `allocate_delivered` + `lock_delivery_capacity` + the two bounds tests; proven green on a fresh DB (51/0/8). Also fixed a pre-existing outbox-migration drift (`backbone-outbox` v2.7.6 `company_id`) that provisioning the DB surfaced.
- **#2 (promote `SellingWriteService`) — BLOCKED, not module-level.** `SellingModule`'s struct + `impl` + the route/AppState composers are **generated** by `metaphor-schema` with no `// <<< CUSTOM` hooks, so the restructure (and the `#[deprecated]` on the 8 fields) is silently overwritten on regen — the generated `build()` emits the deprecation warnings itself. Real fix: a `metaphor-codegen` template change (emit `write_service` + `crud_services()` + deprecation). Framework-level, out of scope for this module.
- **#3 (retire invoice fold) — DEFERRED.** Execution scoping showed it is an architecture migration, not a deletion: retiring `create_sales_invoice` / `post_sales_invoice` / `build_revenue_post` cascades to ~13 tests (7 of 8 golden cases + all 4 GL-seam tests + the `delivery_seam` e2e + 2 `order_to_cash` sites), retires selling's GL seam (`selling_gl.rs` + the `backbone-gl-posting` dep), and supersedes ADR-001/002 ("selling is a GL producer"). Revenue/invoice coverage relocates to `backbone-billing`. Its own dedicated effort.
- **#4 (drop ES scaffold) — BLOCKED, not module-level.** `event_store/` + `subscriptions/` are **generated** for every `backbone-*` module (commit `4bef13a`); deleting them here is reverted by the next regen. Same `metaphor-codegen` template blocker as #2. (The "Reversibility: easy (deletion)" call in the #4 row above was based on the incomplete "unreferenced ⇒ non-generated" read; corrected here.)

**Net: 1 of 4 recommendations was fixable in-module (#1).** #2 and #4 need a `metaphor-codegen` template change; #3 needs a cross-module architecture migration. The council's *diagnosis* holds; the *fixes* are upstream of this module.
