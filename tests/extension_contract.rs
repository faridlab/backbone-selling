...
async fn consumer_rule_rides_domain_event() {
    ...
    w.confirm_sales_order(ok_order, company, &NoUnitCostPort, &NoStockFulfillmentPort).await.unwrap();
    ...
    w.confirm_sales_order(big_order, company, &NoUnitCostPort, &NoStockFulfillmentPort).await.unwrap();
